...
// Map from DF color to ncurses color
 int renderer_curses::ncurses_map_color(int color) {
  if (color < 0) abort();
  switch (color) {
  case 0: return 0;
  case 1: return 4;
...
int renderer_curses::lookup_pair(pair<int,int> color) {
...
void renderer_curses::update_tile(int x, int y) {
...
void renderer_curses::update_all() {
  for (int x = 0; x < init.display.grid_x; x++)
    for (int y = 0; y < init.display.grid_y; y++)
      update_tile(x, y);
}

void renderer_curses::render() {
  refresh();
}

void renderer_curses::resize(int w, int h) {
...
void renderer_curses::grid_resize(int w, int h) {
  gps_allocate(w, h);
}

renderer_curses::renderer_curses() {
  init_curses();
}

bool renderer_curses::get_mouse_coords(int &x, int &y) {
  return false;
}
