...
void textures::upload_textures() {
  std::cerr << "textures::upload_textures\n";

  typedef std::pair< std::uint32_t, std::uint32_t > tile_dimension_t;
  std::map< tile_dimension_t, std::uint32_t > dimensions_count;
  for (SDL_Surface* raw : raws) {
    tile_dimension_t d = std::make_pair(raw->w, raw->h);
    dimensions_count[d]++;
  }

  for (std::pair< tile_dimension_t, std::uint32_t > d : dimensions_count) {
    std::cerr << "dimension: " << d.first.first << "x" << d.first.second << ": " << d.second << "\n";
  }

  if (uploaded)
    return; // Don't bother
  if (!enabler.uses_opengl())
    return; // No uploading
  ...
  for (int i = 0; i < ordered.size(); i++)
    if (catalog_width < ordered[i].w)
      catalog_width = ordered[i].w;
  ...
  if (!npot) {
    ...
    int newx = 1, newy = 1;
    while (newx < catalog_width)
      newx *= 2;
    while (newy < catalog_height)
      newy *= 2;
    catalog_width = newx;
    catalog_height = newy;
    std::cout << "GPU does not support non-power-of-two textures, using " << catalog_width << "x" << catalog_height
        << " catalog.\n";
  }
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, catalog_width, catalog_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, zeroes);
  delete[] zeroes;
  printGLError();
  glBindTexture(GL_TEXTURE_2D, gl_catalog);
  printGLError();
...
SDL_Surface *canonicalize_format(SDL_Surface *src, bool convert_magenta) {
  ...
  tgt = SDL_CreateRGBSurface(SDL_SWSURFACE, src->w, src->h, 32, fmt.Rmask, fmt.Gmask, fmt.Bmask, fmt.Amask);
...
void textures::load_multi_pdim(const string &filename,
                               ...
                               long* disp_x,
                               long* disp_y) {
  std::cerr << "textures::load_multi_pdim " << filename << ", " << dimx << "x" << dimy << "\n";
  ...
  *disp_y = src->h / dimy;
  ...
  for (int y = 0; y < dimy; y++)
    for (int x = 0; x < dimx; x++) {
      SDL_Surface *tile = SDL_CreateRGBSurface(SDL_SWSURFACE,
                                               *disp_x,
                                               *disp_y,
                                               32,
                                               src->format->Rmask,
...
long textures::load(const string &filename, bool convert_magenta) {
  std::cerr << "textures::load " << filename << ", " << convert_magenta << "\n";
