...
namespace widgets {
  ...
  textbox::textbox(::std::string initializer, bool keep) {
    this->keep = keep;
    text = initializer;
  }
  ...
  void textbox::feed(::std::set< interface_key_t > &input) {
    // Backspace
    ...
    // alphabetical order.
    for (::std::set< interface_key_t >::iterator it = input.lower_bound(INTERFACEKEY_STRING_A001); it != input.end() && *it
        <= INTERFACEKEY_STRING_A255; ++it) {
      if (keep == false) {
        keep = true;
        text.clear();
      }
  ...
  void textbox::render(int x1, int x2, int y1, int y2) {
    ...
    gps.addst(text.substr(::std::max(start, 0)));
  }
}
