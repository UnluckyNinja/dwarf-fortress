...
static inline void Resample(SDL_Surface * src, SDL_Surface * dst, int filter) {
  ...
  uint32_t ** temp = new uint32_t *[src->h]; //array of source->height * dest->width
  for (int i = 0; i < src->h; i++)
    temp[i] = new uint32_t[dst->w];
